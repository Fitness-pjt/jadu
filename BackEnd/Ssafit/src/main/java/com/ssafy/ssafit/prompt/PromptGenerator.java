...
public class PromptGenerator {
	...
	public String generatePrompt(UserInfo userInfo) {
	    ...
	    int videoNum = frequency * duration; // 총 받을 비디오 ID 개수
	    int AIkeywordNum = frequency; // AI로부터 추천받을 키워드 개수
	    ...
	    prompt.append("3. 키워드는 쉼표(,)로 구분해주세요.\n");
	    prompt.append("4. 키워드에는 운동 난이도, 부위, 시간 등의 구체적인 정보를 포함해주세요.\n");
	    prompt.append("5. 사용자의 경험 수준에 맞는 난이도의 운동을 추천해주세요.\n");
	    prompt.append("6. 생성된 키워드는 **Python 스타일 코드**가 아닌, **쉼표로 구분된 목록 형태**로 작성해주세요.\n\n");

	    prompt.append("키워드 형식 예시: 초보자 심폐 지구력 홈트 20분, 전신 유산소 운동\n\n");
